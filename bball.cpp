...
void DrawGLScene5()
{
    ...
    const int n = 60;
    Vec vert[n]; 
    Flt angle = 0.0, inc = (PI * 2.0)/(Flt)n;
    for (int i=0; i<n; i++) {
	vert[i][0] = cos(angle); // x value
	vert[i][2] = sin(angle); // z value
	vert[i][1] = 0.0;	 // y value = 0 -> flat
	angle += inc;
    }
    ...
    glLightfv(GL_LIGHT0, GL_POSITION,LightPosition);
    glTranslatef(-1.5f,0.0f,-6.0f);
    glRotatef(rtri/2.0,1.0f,-1.0f,0.0f);
    float col[3] = {0.8f, 1.0f, 0.0f};
	glColor3fv(col);
    glBegin(GL_TRIANGLES);
    ...
    for (int i=0; i<n; i++) {
    	...
    	vecMake(vert[j][0],-1.0f, vert[j][2], v2);
    	vecMake(vert[i][0],-1.0f, vert[i][2], v3);
    	vecSub(v2,v1,v4);
    	vecSub(v3,v1,v5);
	...
	for (int i=0; i<n; i++) {
    	glVertex3f(vert[i][0],-1.0f, vert[i][2]);
	}
	glEnd();
